...
from dataset_tiny import TinyImageNet
from tqdm import tqdm
from torch import nn
import argparse
import torch
import os
from extract_subnet_mov3_tiny import SubMoblieNetV3
...
# python pretrain_backdoor_moblienet_tiny.py --epochs 200 --gpu 0 --lr 0.1
if __name__ == '__main__':
    ...
    out_dir = os.path.join('./tiny_MobileNetV3_models', 'lr_{}'.format(args.lr), 'epochs_{}'.format(args.epochs))
    ...
    model = MobileNetV3_Small(num_classes=200).to(args.device)
    ...
    save_path = os.path.join(out_dir, 'nn_plot_{}_{}_{}.png'.format('tiny', 'MobileNetV3', args.epochs))
    ...
    asr_test, asr_loss_test = test_cifar(model.to(args.device), dataset.malicious_test_dataloader, nn.CrossEntropyLoss(),
                                         args)
    #
    logging.info('BEGIN---------------------- Tiny_MobileNetV3 MODEL ACC ASR ----------------------')
    logging.info("ACC accuracy: {:.6f}\n".format(acc_test))
    logging.info("ASR accuracy: {:.6f}\n".format(asr_test))
    logging.info('COMPLETE---------------------- Tiny_MobileNetV3 MODEL ACC ASR ----------------------')

    logging.info("save model")
    save_model(model, os.path.join(out_dir, f'Pretrained_Backdoored_Tiny_MobileNetV3_{args.epochs}_{args.lr}_' + now_str + '.pt'))