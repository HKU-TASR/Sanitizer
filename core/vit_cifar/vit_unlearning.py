...
if __name__ == '__main__':
    ...
    train_data = DatasetCL(args, full_dataset=dataset_train,
    ...
    acc_test, loss_test = test_cifar(loaded_model.to(args.device), clean_dataset_acc_loader, nn.CrossEntropyLoss(),
                                     args)
    asr_test, asr_loss_test = test_cifar(loaded_model.to(args.device), malicious_dataset_asr_loader,
                                         nn.CrossEntropyLoss(),
                                         args)
    ...
    save_model(loaded_model, os.path.join(out_dir, 'Vit_Cifar10_Real_UL_' + now_str + '.pt'))
    # python vit_unlearning.py --gpu 1 --model_path  --clean_threshold 0.2 --epochs 20 --lr 0.01