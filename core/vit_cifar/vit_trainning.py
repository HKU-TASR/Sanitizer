from architectures.nets_vit_cifar_pure import ViT
import copy
import logging
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=300, help="rounds of FL training")
    ...
    out_dir = os.path.join('./cifar10_vit_models', 'lr_{}'.format(args.lr), 'epochs_{}'.format(args.epochs))
    ...
    vit = ViT(
        ...
        patch_size=2,       # 使用4x4的patch size 或 2x2
    ...
    # plt.savefig('./cifar10_vit_models/nn_plot_{}_{}_{}.png'.format('cifar10', 'vit', args.epochs))
    #######################################################################################################
    ...
    asr_test, asr_loss_test = test_cifar(net.to(args.device), malicious_dataset_asr_loader, nn.CrossEntropyLoss(),
                                         args)
    #
    logging.info('BEGIN---------------------- CIFAR10_vit_base_patch4 MODEL ACC ASR ----------------------')
    logging.info("CIFAR10_vit_base_patch4 ACC accuracy: {:.6f}\n".format(acc_test))
    logging.info("CIFAR10_vit_base_patch4 ASR accuracy: {:.6f}\n".format(asr_test))
    logging.info('COMPLETE---------------------- CIFAR10_vit_base_patch4 MODEL ACC ASR ----------------------')

    logging.info("Save Model")
    save_model(net, os.path.join(out_dir, 'Pretrained_Backdoored_CIFAR10_vit_base_patch4_' + now_str + '.pt'))
    logging.info("-------FINISHED-------\n")
    # python vit_trainning.py --gpu 0
