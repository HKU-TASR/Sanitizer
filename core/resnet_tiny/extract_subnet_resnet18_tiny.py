...
# python extract_subnet_resnet18_tiny.py --gpu 0 --model_path1 --model_path2 --topK_ratio1 0.2
if __name__ == '__main__':
    ...
    out_dir = os.path.join('./extracted_Tiny_Resnet18_structures', 'lr_{}'.format(args.lr), 'epochs_{}'.format(args.epochs))
    ...
    acc_test, loss_test = test_cifar(subnet.to(args.device), dataset.clean_test_dataloader, nn.CrossEntropyLoss(),
                                     args)
    asr_test, asr_loss_test = test_cifar(subnet.to(args.device), dataset.malicious_test_dataloader,
                                         nn.CrossEntropyLoss(),
                                         args)

    #
    logging.info('BEGIN---------------------- subnet_Tiny_Resnet18 MODEL ACC ASR ----------------------')
    logging.info("ACC accuracy: {:.6f}\n".format(acc_test))
    logging.info("ASR accuracy: {:.6f}\n".format(asr_test))
    logging.info('COMPLETE---------------------- subnet_Tiny_Resnet18 MODEL ACC ASR ----------------------')

    logging.info("save subnet_Tiny_Resnet18 model")
    save_model(subnet, os.path.join(out_dir, 'subnet_Tiny_Resnet18_Real_' + str(args.topK_ratio1) + now_str + '.pt'))
